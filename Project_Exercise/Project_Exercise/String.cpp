...
/**********************************************************************/
		 ...
		 These read characters from the input data, and parse them to interpret them as a value of the specific type of its parameter. 
		 The resulting value is stored in the variable passed as parameter.
...
/***********************************************************************/
// Get clarified on string.c_str(), 'const'ness, non-const char array, string, string initialized by a string in quote etc.
/***********************************************************************/

using namespace std;

void ListAndCountWords(string filename) {
	map<string, int> map_doc;
	map<string, int>::iterator it;
	string word;
	ifstream in(filename);
	ofstream out("listcount.txt");
	while (in >> word) {
		map_doc[word]++;
	}
	for( it = map_doc.begin(); it != map_doc.end(); it++) {
		out << it->first << "\t" << it->second << "\n";
	}
}

#define NHASH 29989 // A prime near the size of hash table
#define MULTI 31	// A prime multiplier
vector<class node*> bin(NHASH);
...
public:
	...
	class node* next;
public:
	node(string wrd, int cnt): word(wrd), count(cnt) {
		next = NULL;
	}
};
...
unsigned int hashCode(string word) {
	...
	for(int i = 0; i < len; i++) {
		code = ( MULTI * code + word[0]) % NHASH;
	}
	return code % NHASH;
}

void doTheOp(string word) {
	int code = hashCode(word);
	node *lNode;
	for( lNode = bin[code]; lNode != NULL; lNode = lNode->next) {
		if(word.compare(lNode->word) == 0)
			(lNode->count)++;
		return;
	}
	lNode = new node(word, 1);
	lNode->next = bin[code];	// Part of lnode initialization(Not included in node ctor)
	bin[code] = lNode;
}

void ListAndCountWords1(string filename) {
	...
	string word;
	while (in >> word) {
		// To check if the current word exist in bin
		// Find proper place in the link once it already exists in the bin.
		// If it does, increment the word count and return
		// else, place the current in the bin after creating a new node. 
		doTheOp(word);
	}
	struct node* lNode;
	for (int i = 0; i < NHASH; i++) {
		for( lNode = bin[i]; lNode != NULL; lNode = lNode->next) {
			out << lNode->word << "\t" << lNode->count << "\n";
		}
	}
}
...
char *itoa_local(int n)
{
	// Record sign
	//sign = -(int)((unsigned int)((int)v) >> (sizeof(int) * CHAR_BIT - 1));
	bool negativeOrZero = n >> (sizeof(int) * CHAR_BIT - 1);
	while(1)
	{
		return NULL;
	}
